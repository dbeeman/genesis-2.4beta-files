...
// include definitions of functions to create H-H tabchannels
include hh_tchan

/* hh_tchan.g assigns values to the global variables EREST_ACT,
   ENA, EK, and SOMA_A.  The first three will be superceded by
   values defined below.  The value of SOMA_A set in hh_tchan.g is
   not relevant, as the cell reader calculates the compartment area.
*/
...
// Make a "library element" to hold the prototypes that will be used
...
// The prototype elements in the library are not needed to
// calculate anything, so we disable the library
disable /library

// To ensure that all subsequent elements are made in the library
pushe /library

/* Make a prototype compartment.  The internal fields will be set
   by the cell reader, so they do not need to be set here.  The
...
// Make a prototype excitatory channel, "Ex_channel" (in synchans.g)
...
// Make spike generator (spikegen) element "spike" (in protospike.g)
make_spike
